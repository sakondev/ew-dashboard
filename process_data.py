...
def process_sales_data():
    ...
    รวมถึงการวิเคราะห์รายสาขา, รายเดือน, เปรียบเทียบเดือน, ปัญหาขาดสต็อก
    และเพิ่มข้อมูลเชิงลึกรายเดือนสำหรับ Best Sellers/Slow Movers
    """
    ...
    for filename, month_year_str in sales_files_info.items():
        try:
            df = pd.read_csv(filename, encoding='utf-8') # Default to utf-8
            missing_original_sales_cols = [orig_col for orig_col in sales_col_mapping.keys() if orig_col not in df.columns]
            if missing_original_sales_cols:
                print(f"คำเตือน: ไฟล์ขาย {filename} ขาดคอลัมน์ดั้งเดิม: {', '.join(missing_original_sales_cols)}. จะข้ามไฟล์นี้")
                continue
            df.rename(columns=sales_col_mapping, inplace=True)
            if not all(col in df.columns for col in required_sales_cols_renamed):
                missing_renamed = [col for col in required_sales_cols_renamed if col not in df.columns]
                print(f"คำเตือน: ไฟล์ขาย {filename} หลัง rename ขาดคอลัมน์ที่จำเป็น: {', '.join(missing_renamed)}. จะข้ามไฟล์นี้")
                continue
            ...
            try: # Try another common Thai encoding if utf-8 fails
                df = pd.read_csv(filename, encoding='iso-8859-11')
                # Repeat column check and processing if successful
                missing_original_sales_cols = [orig_col for orig_col in sales_col_mapping.keys() if orig_col not in df.columns]
                if missing_original_sales_cols: continue # Skip if still problematic
                df.rename(columns=sales_col_mapping, inplace=True)
                ...
                df['revenue'] = pd.to_numeric(df['revenue'], errors='coerce').fillna(0)
                all_sales_data.append(df[required_sales_cols_renamed + ['month_year']])
                print(f"ข้อมูล: อ่านไฟล์ขาย {filename} ด้วย encoding 'iso-8859-11' สำเร็จ")
            except Exception as e_fallback:
                print(f"คำเตือน: ไม่สามารถอ่านไฟล์ขาย {filename} ด้วย encoding 'utf-8' หรือ 'iso-8859-11'. ข้อผิดพลาด: {e_fallback}. จะข้ามไฟล์นี้")
        except Exception as e: print(f"เกิดข้อผิดพลาดในการอ่านไฟล์ขาย {filename}: {e}")

    if not all_sales_data:
        print("ข้อผิดพลาด: ไม่สามารถโหลดข้อมูลการขายได้เลย")
        return None
    ...
    for filename, month_year_str in stock_files_info.items():
        try:
            df_stock = pd.read_csv(filename, encoding='utf-8') # Default to utf-8
            ...
            df_stock['month_year'] = month_year_str
            df_stock['on_hand'] = pd.to_numeric(df_stock['on_hand'], errors='coerce').fillna(0)
            ...
            try: # Try another common Thai encoding
                ...
                df_stock['on_hand'] = pd.to_numeric(df_stock['on_hand'], errors='coerce').fillna(0)
                all_stock_data.append(df_stock[required_stock_cols_renamed + ['month_year']])
                print(f"ข้อมูล: อ่านไฟล์สต็อก {filename} ด้วย encoding 'iso-8859-11' สำเร็จ")
            except Exception as e_fallback:
                print(f"คำเตือน: ไม่สามารถอ่านไฟล์สต็อก {filename} ด้วย encoding 'utf-8' หรือ 'iso-8859-11'. ข้อผิดพลาด: {e_fallback}. จะข้ามไฟล์นี้")
    ...
    monthly_sales_agg_for_products = sales_df.groupby(['month_year', 'item_code', 'item_name']).agg(
        total_quantity_sold=('quantity', 'sum'), total_revenue=('revenue', 'sum')
    ).reset_index()
    ...
    if 'item_name_x' in master_df.columns:
         master_df['item_name'] = master_df['item_name_x'].fillna(master_df['item_code'].map(item_name_map))
         master_df.drop(columns=['item_name_x', 'item_name_y'], inplace=True, errors='ignore')
    ...
    if not stock_df.empty:
        ...
        stock_df_shifted['month_year_dt'] = pd.to_datetime(stock_df_shifted['month_year'] + '-01', errors='coerce')
        stock_df_shifted = stock_df_shifted.dropna(subset=['month_year_dt'])
        ...
        master_df.rename(columns={'on_hand': 'eom_stock'}, inplace=True)
        master_df.drop(columns=['prev_month_year'], inplace=True, errors='ignore')
    ...
    products_data_list = []
    overall_sales_by_product = master_df.groupby(['item_code', 'item_name']).agg(
        overall_quantity=('total_quantity_sold', 'sum'), overall_revenue=('total_revenue', 'sum'),
        avg_bom_stock=('bom_stock', lambda x: convert_value(x[x > 0].mean()) if (x > 0).any() else 0)
    ).reset_index()
    
    best_sellers_overall_df = overall_sales_by_product.sort_values(by='overall_revenue', ascending=False).head(10)
    num_months_data = len(active_months) if active_months else 1
    overall_sales_by_product['avg_monthly_sales'] = overall_sales_by_product['overall_quantity'] / num_months_data
    slow_movers_overall_df = overall_sales_by_product[
        (overall_sales_by_product['avg_monthly_sales'] < 10) & (overall_sales_by_product['avg_bom_stock'] > 0)
    ].sort_values(by='avg_monthly_sales', ascending=True).head(10)
    
    potential_stock_issues = [] 
    for item_code_val in product_codes:
        product_df_item = master_df[master_df['item_code'] == item_code_val].sort_values(by='month_year')
        item_name_val = product_df_item['item_name'].iloc[0] if not product_df_item.empty and product_df_item['item_name'].iloc[0] != 'N/A' else item_code_val
        monthly_data_for_product = []
        product_df_item['prev_month_quantity_sold'] = product_df_item['total_quantity_sold'].shift(1)
        product_df_item['prev_month_eom_stock'] = product_df_item['eom_stock'].shift(1)
        for _, row in product_df_item.iterrows():
            analysis_text, current_sales, prev_sales, bom_stock, eom_stock, prev_eom_stock = "ข้อมูลไม่เพียงพอ", row['total_quantity_sold'], row['prev_month_quantity_sold'], row['bom_stock'], row['eom_stock'], row['prev_month_eom_stock']
            issue_key = f"{item_code_val}_{row['month_year']}"
            # (Detailed analysis logic from previous version - kept for brevity in this diff, assume it's correct)
            if pd.isna(prev_sales): 
                analysis_text = "เดือนแรกของข้อมูลสินค้า"
                if bom_stock == 0 and current_sales > 0: analysis_text += " (เริ่มขายได้แม้ BOM=0)"
                elif bom_stock == 0 and current_sales == 0:
                     analysis_text += " (ไม่มีสต็อกต้นเดือนและไม่มีการขาย)"
                     if not any(p['issueKey'] == issue_key for p in potential_stock_issues): potential_stock_issues.append({"itemCode": item_code_val, "itemName": item_name_val, "monthYear": row['month_year'], "issue": "สินค้าหมดสต็อก (BOM=0) และไม่มีการขาย", "issueKey": issue_key})
            elif current_sales > prev_sales:
                analysis_text = "ยอดขายเพิ่มขึ้น"
                if not pd.isna(prev_eom_stock) and prev_eom_stock == 0: analysis_text += " อาจหลังเติมสต็อก"
            elif current_sales < prev_sales:
                analysis_text = "ยอดขายลดลง"
                if bom_stock == 0 and current_sales == 0: 
                    analysis_text += " สินค้าหมดสต็อก (BOM=0)"
                    if not any(p['issueKey'] == issue_key for p in potential_stock_issues): potential_stock_issues.append({"itemCode": item_code_val, "itemName": item_name_val, "monthYear": row['month_year'], "issue": "สินค้าหมดสต็อก (BOM=0) และไม่มีการขาย", "issueKey": issue_key})
                elif bom_stock > 0 and eom_stock == 0 and current_sales > 0: 
                    analysis_text += " สินค้าอาจหมดระหว่างเดือน (EOM=0)"
                    if not any(p['issueKey'] == issue_key for p in potential_stock_issues): potential_stock_issues.append({"itemCode": item_code_val, "itemName": item_name_val, "monthYear": row['month_year'], "issue": "สินค้าอาจหมดระหว่างเดือน (EOM=0)", "issueKey": issue_key})
            elif current_sales == 0:
                analysis_text = "ยอดขายเป็นศูนย์"
                if bom_stock == 0: 
                    analysis_text += " สินค้าหมดสต็อก"
                    if not any(p['issueKey'] == issue_key for p in potential_stock_issues): potential_stock_issues.append({"itemCode": item_code_val, "itemName": item_name_val, "monthYear": row['month_year'], "issue": "ยอดขายเป็นศูนย์ สินค้าหมดสต็อก", "issueKey": issue_key})
    ...
    branch_analysis_output = {} # (Branch analysis logic from previous version)
    if 'branch' in sales_df.columns:
        ...
        branch_analysis_output['overallSalesByBranch'] = [{"branch": row['branch'], "total_revenue": convert_value(row['total_revenue']), "total_quantity": convert_value(row['total_quantity'])} for _, row in overall_sales_by_branch_df.iterrows()]
        ...
        monthly_sales_by_branch_for_chart = []
        for branch_name, group in monthly_sales_by_branch_agg.groupby('branch'):
            ...
            monthly_sales_by_branch_for_chart.append({"branch": branch_name, "monthlyData": monthly_data})
        branch_analysis_output['monthlySalesTrendByBranch'] = monthly_sales_by_branch_for_chart
        top_products_per_branch_list = []
        branch_item_sales = sales_df.groupby(['branch', 'item_code', 'item_name']).agg(revenue_sum=('revenue', 'sum'), quantity_sum=('quantity', 'sum')).reset_index()
        for branch_name, group in branch_item_sales.groupby('branch'):
            top_prods_df = group.sort_values(by='revenue_sum', ascending=False).head(5)
            top_products_per_branch_list.append({"branch": branch_name, "topProducts": [{"item_code": r['item_code'], "item_name": r['item_name'], "revenue": convert_value(r['revenue_sum']), "quantity": convert_value(r['quantity_sum'])} for _, r in top_prods_df.iterrows()]})
        branch_analysis_output['topProductsPerBranch'] = top_products_per_branch_list
    else: branch_analysis_output = {'overallSalesByBranch': [], 'monthlySalesTrendByBranch': [], 'topProductsPerBranch': []}


    monthly_summary_analysis_list = [] # (Monthly summary logic from previous version)
    if not master_df.empty and active_months:
        for month in active_months:
            month_sales_df = sales_df[sales_df['month_year'] == month]
    ...
    mom_comparison_list = [] # (MoM comparison logic from previous version)
    if len(active_months) > 1:
        ...
        overall_monthly_sales['prev_revenue'] = overall_monthly_sales['total_revenue'].shift(1)
        overall_monthly_sales['prev_quantity'] = overall_monthly_sales['total_quantity'].shift(1)
    ...
    # --- NEW: Monthly Analysis Insights (Best Sellers, Slow Movers per month) ---
    monthly_analysis_insights_dict = {}
    if not master_df.empty and active_months:
        for month in active_months:
            month_master_df = master_df[master_df['month_year'] == month]
            
            # Best Sellers for the month
            month_sales_agg = month_master_df.groupby(['item_code', 'item_name']).agg(
                metric_revenue=('total_revenue', 'sum'),
                metric_quantity=('total_quantity_sold', 'sum') # Use total_quantity_sold from master_df for the month
            ).reset_index()
            best_sellers_month_df = month_sales_agg.sort_values(by='metric_revenue', ascending=False).head(10)
            
            # Slow Movers for the month
            # For single month, avg_monthly_sales is just total_quantity_sold
            # avg_bom_stock is just bom_stock for that month
            month_master_df_for_slow = month_master_df.copy() # Avoid SettingWithCopyWarning
            month_master_df_for_slow['metric_avg_sales'] = month_master_df_for_slow['total_quantity_sold']
            month_master_df_for_slow['metric_avg_stock'] = month_master_df_for_slow['bom_stock']

            slow_movers_month_df = month_master_df_for_slow[
                (month_master_df_for_slow['metric_avg_sales'] < 10) & (month_master_df_for_slow['metric_avg_stock'] > 0)
            ].sort_values(by='metric_avg_sales', ascending=True).head(10)

            monthly_analysis_insights_dict[month] = {
                "bestSellers": [{"item_code": r['item_code'], "item_name": r['item_name'], "metric_revenue": convert_value(r['metric_revenue']), "metric_quantity": convert_value(r['metric_quantity'])} for _, r in best_sellers_month_df.iterrows()],
                "slowMovers": [{"item_code": r['item_code'], "item_name": r['item_name'], "metric_avg_sales": convert_value(r['metric_avg_sales']), "metric_avg_stock": convert_value(r['metric_avg_stock'])} for _, r in slow_movers_month_df.iterrows()]
            }

    key_indicators = [] # (Key indicators logic from previous version)
    ...
    if not stock_df.empty and total_quantity_all > 0 and not master_df['bom_stock'].dropna().empty:
        ...
        if avg_total_bom_stock_per_month > 0:
            stock_turnover_ratio = convert_value(total_quantity_all / avg_total_bom_stock_per_month)
            key_indicators.append({"name": "อัตราส่วนหมุนเวียนสินค้าคงคลัง (ภาพรวม)", "value": f"{stock_turnover_ratio:.2f} รอบ", "details": "คำนวณจาก: ยอดขายรวม (จำนวนชิ้น) / สต็อกเฉลี่ยต้นเดือนรวมทุกสินค้า (เฉพาะเดือนที่มีข้อมูล)"})
        else: key_indicators.append({"name": "อัตราส่วนหมุนเวียนสินค้าคงคลัง (ภาพรวม)", "value": "N/A", "details": "ไม่สามารถคำนวณได้ (สต็อกเฉลี่ยเป็นศูนย์หรือไม่มีข้อมูลสต็อก)"})
    else: key_indicators.append({"name": "อัตราส่วนหมุนเวียนสินค้าคงคลัง (ภาพรวม)", "value": "N/A", "details": "ไม่มีข้อมูลสต็อกเพียงพอสำหรับการคำนวณ"})
    if not stock_df.empty and total_quantity_all > 0 and active_months and not master_df.empty:
        first_month = active_months[0]
        initial_stock_sum_for_sell_through = master_df[master_df['month_year'] == first_month]['bom_stock'].sum()
        denominator_sell_through = total_quantity_all + initial_stock_sum_for_sell_through
        if denominator_sell_through > 0 :
            sell_through_rate = convert_value((total_quantity_all / denominator_sell_through) * 100)
            key_indicators.append({"name": "อัตราการขายสินค้าผ่าน (Sell-Through Rate - ภาพรวม)", "value": f"{sell_through_rate:.2f}%", "details": "คำนวณจาก: (ยอดขายรวม (จำนวนชิ้น) / (ยอดขายรวม (จำนวนชิ้น) + สต็อกรวมต้นงวดเดือนแรก)) * 100"})
        else: key_indicators.append({"name": "อัตราการขายสินค้าผ่าน (Sell-Through Rate - ภาพรวม)", "value": "N/A", "details": "ไม่สามารถคำนวณได้ (ผลรวมยอดขายและสต็อกเริ่มต้นเป็นศูนย์ หรือไม่มีข้อมูลสต็อก)"})
    else: key_indicators.append({"name": "อัตราการขายสินค้าผ่าน (Sell-Through Rate - ภาพรวม)", "value": "N/A", "details": "ไม่มีข้อมูลสต็อกหรือยอดขายเพียงพอสำหรับการคำนวณ"})

    output_json = {
        "summary": {"totalRevenue": convert_value(total_revenue_all), "totalQuantitySold": convert_value(total_quantity_all), "activeMonths": active_months, "dataSourceNote": "ข้อมูลประมวลผลจากไฟล์ CSV (เพิ่มการเลือกเดือนสำหรับข้อมูลเชิงลึก)"},
        "monthlySalesTrend": monthly_sales_trend_for_chart_cleaned,
        "productsData": products_data_list,
        "analysisInsights": { # This remains for OVERALL insights
            "bestSellers": [{"item_code": row['item_code'], "item_name": row['item_name'], "metric_revenue": convert_value(row['overall_revenue']), "metric_quantity": convert_value(row['overall_quantity'])} for _, row in best_sellers_overall_df.iterrows()],
            "slowMovers": [{"item_code": row['item_code'], "item_name": row['item_name'], "metric_avg_sales": convert_value(row['avg_monthly_sales']), "metric_avg_stock": convert_value(row['avg_bom_stock'])} for _, row in slow_movers_overall_df.iterrows()],
            "potentialStockIssues": potential_stock_issues 
        },
        "monthlyAnalysisInsights": monthly_analysis_insights_dict, # NEW: For month-specific insights
        "branchAnalysis": branch_analysis_output,
...
if __name__ == '__main__':
    ...
    if dashboard_data:
        try:
            ...
            # Uncomment to write to file directly
        ...
        print("Failed to generate dashboard data (process_sales_data returned None).")
